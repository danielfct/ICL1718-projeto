...
public class ParserTester {
	...
	public void testFun() throws Exception {
		...
		testCase("fun x:int, y:int -> x + y end");
		testCase("fun x:int, f:funt(int, int) -> f(x) end");
		testNegativeCase("f x -> x end");
		testNegativeCase("fun x -> x end");
		testNegativeCase("fun x:int => x end");
		testNegativeCase("fun x:funt -> x end");
	}
	...
	public void testForEach() throws Exception {
		testCase("foreach e in [1] do e+1 end");
		testCase("foreach e in [1,2] do e+1 end");
		testCase("foreach e in [1,2,3,4,5] do e+1 end");
		testCase("foreach elem in [true,false,true,false,true] do elem || true end");
		testCase("foreach e in [] do 1 end");
		testCase("foreach e in [var(1), var(2)] do if *e == 1 then var(2) else var(1) end end");
		testCase("foreach e in [fun x:int -> x+1 end, fun x:int -> x-1 end] do fun x:int -> x+e(1) end end");
		testNegativeCase("foreach e in 1,2 do e*5 end");
	}
